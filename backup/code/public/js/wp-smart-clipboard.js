    ...
    if (window.wp && window.wp.blocks && window.wp.data) {
      try {
        ...
        if (blocks && blocks.length > 0) {
          const blockEditor = dispatch('core/block-editor') || dispatch('isolated/editor');
  ...
  function appendBlockToGutenberg(htmlSnippet, label) {
    ...
    if (window.wp && window.wp.blocks && window.wp.data) {
      try {
        ...
        const blockEditor = dispatch('core/block-editor') || dispatch('isolated/editor');
        const currentBlocks = (select('core/block-editor') ? select('core/block-editor').getBlocks() : []) || [];
        ...
        if (newBlocks && newBlocks.length > 0 && blockEditor && blockEditor.resetBlocks) {
