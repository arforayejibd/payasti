 ...
 */
function getEmailVerificationEmailHtml({ name, verifyUrl }) {
  const currentYear = new Date().getFullYear();
  const contactEmail = (CONTACT && CONTACT.email) ? CONTACT.email : 'payastimag@gmail.com';
  const logoUrl = `${SITE_URL}/images/Payasti-logo.png`;
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>ইমেইল ঠিকানা যাচাইকরণ | ${SITE_NAME}</title>
  <style>
    body, table, td, a { -webkit-text-size-adjust: 100%; -ms-text-size-adjust: 100%; }
    table, td { mso-table-lspace: 0pt; mso-table-rspace: 0pt; }
    img { -ms-interpolation-mode: bicubic; border: 0; outline: none; text-decoration: none; }
...
<body style="margin: 0; padding: 0; background-color: #f4f7f5;">
  <div style="display: none; font-size: 1px; color: #fefefe; line-height: 1px; max-height: 0px; max-width: 0px; opacity: 0; overflow: hidden;">
    আপনার ${SITE_NAME} লেখক অ্যাকাউন্টের ইমেইল ভেরিফাই করার লিংক।
  </div>
        ...
        <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="600" class="email-container" style="background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 20px rgba(7, 86, 3, 0.08); border: 1px solid #e2ece0;">
          ...
          <tr>
            <td class="content-cell" style="padding: 36px 40px; background-color: #ffffff;">
              ...
              <p style="margin: 0 0 16px; font-size: 16px; line-height: 1.7; color: #4a5568;">
                স্বাগতম প্রিয় <strong>${name || 'লেখক'}</strong>,
              </p>

              <p style="margin: 0 0 24px; font-size: 16px; line-height: 1.7; color: #4a5568;">
                <strong>${SITE_NAME}</strong>-এ লেখক হিসেবে রেজিস্ট্রেশন করার জন্য আপনাকে ধন্যবাদ। আপনার ইমেইল ঠিকানাটি নিশ্চিত করতে নিচের বাটনে ক্লিক করুন:
              </p>
              ...
              <table role="presentation" border="0" cellpadding="0" cellspacing="0" style="margin: 28px 0;">
                <tr>
                  <td align="center" style="border-radius: 8px; background: linear-gradient(135deg, #075603 0%, #068200 100%);">
                    <a href="${verifyUrl}" target="_blank" class="btn-action" style="font-size: 17px; font-weight: 700; color: #ffffff; text-decoration: none; padding: 14px 34px; display: inline-block; border-radius: 8px; letter-spacing: 0.3px; box-shadow: 0 4px 12px rgba(7, 86, 3, 0.25);">
                      ইমেইল যাচাই সম্পন্ন করুন
                    </a>
                  </td>
                </tr>
              </table>
              ...
              <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="background-color: #f7faf7; border-left: 4px solid #075603; border-radius: 4px; margin: 24px 0;">
                <tr>
                  <td style="padding: 14px 16px;">
                    <p style="margin: 0; font-size: 14px; line-height: 1.6; color: #2d3748;">
                      ⏰ <strong>সময়সীমা:</strong> নিরাপত্তার স্বার্থে এই লিংকটি আগামী <strong>২৪ ঘণ্টা</strong>র জন্য কার্যকর থাকবে।
                    </p>
                  </td>
                </tr>
              </table>
              ...
              <!-- Fallback Link Section -->
              <p style="margin: 24px 0 8px; font-size: 14px; line-height: 1.6; color: #718096;">
                বাটনে ক্লিক করতে সমস্যা হলে নিচের লিংকটি কপি করে ব্রাউজারে খুলুন:
              </p>
              <div style="background-color: #edf2f7; padding: 10px 14px; border-radius: 6px; word-break: break-all; font-size: 13px; color: #2b6cb0; line-height: 1.5;">
                <a href="${verifyUrl}" target="_blank" style="color: #075603; text-decoration: underline;">${verifyUrl}</a>
              </div>

              <div style="margin-top: 30px; padding-top: 20px; border-top: 1px dashed #e2e8f0;">
                <p style="margin: 0; font-size: 13px; line-height: 1.6; color: #a0aec0;">
                  🔒 আপনি যদি পয়স্তিতে নিবন্ধনের চেষ্টা না করে থাকেন, তবে এই ইমেইলটি উপেক্ষা করতে পারেন।
                </p>
              </div>
            </td>
          </tr>
 ...
 */
async function sendEmailVerificationMail(user, verifyUrl) {
  const mailHtml = getEmailVerificationEmailHtml({
    name: user.display_name || user.username,
    verifyUrl
  });
  ...
  const mailOptions = {
    from: process.env.SMTP_FROM || `"${SITE_NAME}" <${CONTACT && CONTACT.email ? CONTACT.email : 'noreply@payasti.com'}>`,
    to: user.email,
    subject: `আপনার ইমেইল ঠিকানা যাচাই করুন - ${SITE_NAME}`,
    html: mailHtml,
    text: `প্রিয় ${user.display_name || user.username},\n\n${SITE_NAME}-এ রেজিস্ট্রেশনের জন্য ধন্যবাদ। ইমেইল ভেরিফাই করতে নিচের লিংকে প্রবেশ করুন:\n${verifyUrl}\n\nএই লিংকটি ২৪ ঘণ্টা কার্যকর থাকবে।`
  };

  if (transporter) {
    try {
      const info = await transporter.sendMail(mailOptions);
      console.log(`[MAIL] Verification email sent to ${user.email} (MessageId: ${info.messageId})`);
      return { success: true, messageId: info.messageId };
    } catch (error) {
      console.error('[MAIL ERROR] Failed to send verification email:', error);
      console.log(`[MAIL FALLBACK] Verification URL for ${user.email}: ${verifyUrl}`);
      return { success: false, error: error.message, verifyUrl };
    }
  } else {
    console.log('=====================================================');
    console.log(`[MAIL DEV MODE] Email Verification Link for: ${user.email}`);
    console.log(`User: ${user.display_name} (@${user.username})`);
...
/**
 * Send account approved notification email to user
 */
async function sendAccountApprovedEmail(user) {
  ...
  const mailOptions = {
    from: process.env.SMTP_FROM || `"${SITE_NAME}" <${CONTACT && CONTACT.email ? CONTACT.email : 'noreply@payasti.com'}>`,
    to: user.email,
    subject: `অভিনন্দন! আপনার লেখক অ্যাকাউন্ট অনুমোদিত হয়েছে - ${SITE_NAME}`,
    html: `
      <div style="font-family: 'Hind Siliguri', sans-serif; padding: 24px; color: #2d3748;">
        <h2 style="color: #075603;">অভিনন্দন প্রিয় ${user.display_name || user.username}!</h2>
        <p>পয়স্তি ম্যাগাজিনে আপনার লেখক অ্যাকাউন্টটি অ্যাডমিন কর্তৃক অনুমোদিত হয়েছে।</p>
        <p>এখন থেকে আপনি লগইন করে আপনার সাহিত্যকর্ম (কবিতা, গল্প, প্রবন্ধ ইত্যাদি) সাবমিট করতে পারবেন।</p>
        <p><a href="${loginUrl}" style="background: #075603; color: #fff; padding: 10px 20px; text-decoration: none; border-radius: 6px; display: inline-block;">লগইন করুন</a></p>
        <p style="margin-top: 30px; font-size: 13px; color: #718096;">ধন্যবাদান্তে,<br>${SITE_NAME} টিম</p>
      </div>
    `,
    text: `অভিনন্দন ${user.display_name || user.username}!\nআপনার লেখক অ্যাকাউন্টটি অনুমোদিত হয়েছে। লগইন করুন: ${loginUrl}`
  };
  ...
  if (transporter) {
    try {
      ...
      console.warn('[MAIL ERROR] Failed to send account approval email:', error.message);
    }
  }
}
