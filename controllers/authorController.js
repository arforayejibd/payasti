...
exports.getAuthorsList = (req, res) => {
  ...
  const page = parseInt(req.params.page || req.query.page || req.query.paged) || 1;
  ...
  // Get total authors count overall (matching WordPress count_users())
  const totalAuthorsOverallRow = db.prepare("SELECT COUNT(*) AS total FROM users WHERE role IN ('author', 'editor', 'admin')").get();
  const totalAuthorsOverall = totalAuthorsOverallRow ? totalAuthorsOverallRow.total : 93;

  const totalFilteredRow = db.prepare(countSql).get(...countParams);
  ...
  const authors = db.prepare(usersSql).all(...usersParams);

  // For each author, resolve avatar (matching WordPress Gravatar sha256 &d=mm&r=g) and fetch their top 3 published writings
  const crypto = require('crypto');
  const getAuthorPosts = db.prepare(`
  ...
  authors.forEach(author => {
    ...
    author.recentPosts = getAuthorPosts.all(author.id);
  });
...
exports.getAuthorProfile = (req, res) => {
  const { slug } = req.params;
  const page = parseInt(req.query.page) || 1;
  const limit = 20;
  const offset = (page - 1) * limit;

  const author = db.prepare('SELECT * FROM users WHERE nicename = ? OR username = ?').get(slug, slug);
  ...
  const totalPostsRow = db.prepare("SELECT COUNT(*) AS total FROM posts WHERE author_id = ? AND status = 'publish'").get(author.id);
